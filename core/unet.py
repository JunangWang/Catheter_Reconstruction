...
class MVAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.contiguous().contiguous().permute(0, 2, 3, 1).contiguous().reshape(B, -1, VC).contiguous()
        x = self.attn(x)
        x = x.contiguous().reshape(B, H, W, VC).contiguous().permute(0, 3, 1, 2).contiguous()
...
class UpBlock(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            ...
            cskip = prev_out_channels if (i == num_layers - 1) else out_channels # cskip is the downblock output dimension
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # first
        x = self.conv_in(x.contiguous())
        
        # down
        xss = [x]
        for block in self.down_blocks:
            x, xs = block(x.contiguous())
            xss.extend(xs)
        
        # mid
        x = self.mid_block(x.contiguous())

        # up
        for block in self.up_blocks:
            xs = xss[-len(block.nets):]
            xss = xss[:-len(block.nets)]
            x = block(x.contiguous(), xs)

        # last
        x = self.norm_out(x.contiguous())
