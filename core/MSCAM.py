...
class CAB(nn.Module):
    def __init__(self, in_channels, out_channels=None, ratio=16, activation='relu'):
        '''
        chaneel attention block
        x -> avg pool -> conv  -> ReLU -> conv -> x1
        x -> max pool -> conv  -> ReLU -> conv -> x2
        out = sigmoid(x1 + x2)
        out = out * x
        '''
...
class SAB(nn.Module):
    def __init__(self, kernel_size=7):
        ...
        x -> channel avg pool -> x1
        x -> channel max pool -> x2
        x3 = cat(x1, x2)
        x3 = large kernel conv(x3)
        out = sigmoid(x3)
        out = out * x
        '''
...
class MSCB(nn.Module):
    ...
    x -> expand channels -> pconv1 -> BNorm -> ReLU6 -> MSDC -> pconv2 -> BNorm -> skip connection
    """
...
class MSCAM(nn.Module):
    def __init__(self, in_channels, out_channels, n=1, stride=1, kernel_sizes=[1,3,5], expansion_factor=2, dw_parallel=True, add=True, activation='relu6'):
        """
        ...
        self.mscb = MSCBLayer(in_channels, out_channels, n, stride, kernel_sizes, expansion_factor, dw_parallel, add, activation)
